...
public class FetcherReducer extends
  ...
  private static class FetchItemQueues {
    ...
    public FetchItemQueues(Configuration conf) throws IOException {
      ...
      this.minCrawlDelay =
          (long) (conf.getFloat("fetcher.server.min.delay", 0.0f) * 1000);
      ...
      if (fiq == null) {
        ...
        if (useHostSettings) {
          ...
          try {
            ...
            if (host != null) {
              fiq =
                  new FetchItemQueue(conf, host.getInt("q_mt", maxThreads),
  ...
  private void reportAndLogStatus(Context context, float actualPages,
    ...
    status.append(Math.round((((float) pages.get()) * 10) / elapsed) / 10.0)
        .append(" ");
    status.append(Math.round(((float) actualPages) * 10) / 10.0).append(
        " pages/s, ");
  ...
  public void run(Context context) throws IOException, InterruptedException {
    ...
    feeder =
        new QueueFeeder(context, fetchQueues, threadCount * maxFeedPerThread);
    feeder.start();
    ...
    int throughputThresholdPages =
        conf.getInt("fetcher.throughput.threshold.pages", -1);
    if (LOG.isInfoEnabled()) {
      LOG.info("Fetcher: throughput threshold: " + throughputThresholdPages);
    }
    int throughputThresholdSequence =
        conf.getInt("fetcher.throughput.threshold.sequence", 5);
    if (LOG.isInfoEnabled()) {
      LOG.info("Fetcher: throughput threshold sequence: "
          + throughputThresholdSequence);
    }
    long throughputThresholdTimeLimit =
        conf.getLong("fetcher.throughput.threshold.check.after", -1);
