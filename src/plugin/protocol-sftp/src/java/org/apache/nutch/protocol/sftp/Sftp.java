...
public class Sftp implements Protocol {
  ...
  private static final Map<String, BlockingQueue<ChannelSftp>> channelSftpByHostMap =
      new Hashtable<String, BlockingQueue<ChannelSftp>>();
  ...
  public ProtocolOutput getProtocolOutput(String url, WebPage page) {
    ...
    try {
      ...
      if (urlFile.endsWith(".htm") || urlFile.endsWith(".html")) {
        ProtocolOutput po =
            getFileProtocolOutput(sUrl, channelSftp, "text/html");
        return po;
      } else if (urlFile.endsWith(".pdf")) {
        ProtocolOutput po =
            getFileProtocolOutput(sUrl, channelSftp, "application/pdf");
        return po;
      } else {
  ...
  private ProtocolOutput getFileProtocolOutput(URL url,
    ...
    Content content =
        new Content(urlStr, urlStr, bytes, contentType, metadata, configuration);
  ...
  private ProtocolOutput getDirectoryProtocolOutput(URL url,
    ...
    try {
      ...
      Content content =
          new Content(url.toExternalForm(), url.toExternalForm(),
  ...
  public void setConf(Configuration arg0) {
    ...
    synchronized (channelSftpByHostMap) {
      ...
      BlockingQueue<ChannelSftp> queue =
          new ArrayBlockingQueue<ChannelSftp>(1, true);
      try {
        queue.put(cSftp);
      } catch (InterruptedException e) {
