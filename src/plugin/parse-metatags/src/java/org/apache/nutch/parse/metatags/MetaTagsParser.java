...
public class MetaTagsParser implements ParseFilter {
   ...
   */
  private void addIndexedMetatags(Map<Utf8, ByteBuffer> metadata,
  ...
  public Parse filter(String url, WebPage page, Parse parse,
    ...
    for (Entry<Utf8, ByteBuffer> entry : page.getMetadata().entrySet()) {
      String mdName = entry.getKey().toString();
      ByteBuffer byteV = entry.getValue();
      String value = Bytes.toStringBinary(byteV.array());
      addIndexedMetatags(metadata, mdName, value);
    }

    // add temporary metadata to page metadata
    for (Entry<Utf8, ByteBuffer> entry : metadata.entrySet()) {
      Utf8 key = entry.getKey();
      // ByteBuffer bb = ByteBuffer.allocate(key.length());
      // bb.put(key.getBytes());
      page.getMetadata().put(key, entry.getValue());
    }
